...
/**
 * APP-78 — Downloads podcast audio from YouTube (via yt-dlp) or HTTP (RSS enclosures),
 * then converts to 16 kHz mono WAV using ffmpeg.
 *
 * <p>Both yt-dlp and ffmpeg must be installed and available on {@code PATH} before the
 * application starts. A startup check ({@link #validateDependencies()}) logs a fatal
 * error if either binary is missing.
 */
@Service
public class AudioService {
    ...
    public AudioService(
            @Value("${localloom.audio.dir:data/audio}") String audioDir,
     ...
     */
    public Path downloadAndConvert(String audioUrl, UUID contentUnitId, boolean isYoutube) {
        log.info("Starting download+convert pipeline for contentUnit={} url={} youtube={}",
                contentUnitId, audioUrl, isYoutube);

        Path downloaded = isYoutube
     ...
     */
    public Path downloadYoutube(String url, UUID contentUnitId) {
        Path outputPath = audioDir.resolve(contentUnitId + "_raw.wav");
    ...
    /**
     * Downloads an HTTP audio enclosure (e.g. from an RSS feed) by streaming the
     * response body directly to disk.
     */
    public Path downloadHttp(String audioUrl, UUID contentUnitId) {
        String extension = deriveExtension(audioUrl);
        Path outputPath = audioDir.resolve(contentUnitId + "_raw" + extension);
     ...
     *
     * <p>Command: {@code ffmpeg -y -i <input> -ar 16000 -ac 1 -f wav <output>}
     */
    public Path convertToWav(Path inputFile, UUID contentUnitId) {
        Path outputPath = audioDir.resolve(contentUnitId + ".wav");
        List<String> command = List.of(
                "ffmpeg",
                "-y",
     ...
     *
     * @throws AudioServiceException if either binary is missing or fails to start
     */
    ...
    private void runWithRetry(List<String> command, Path outputPath, String description) {
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    private void checkBinary(String binary) {
