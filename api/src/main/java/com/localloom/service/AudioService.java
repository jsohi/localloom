...
/**
 * Downloads podcast audio from YouTube (via yt-dlp) or HTTP (RSS enclosures), then converts to 16
 * kHz mono WAV using ffmpeg.
 */
@Service
public class AudioService {
   ...
   */
  public Path downloadAndConvert(
      final String audioUrl, final UUID contentUnitId, final boolean isYoutube) {
    log.info(
        "Starting download+convert pipeline for contentUnit={} url={} youtube={}",
        contentUnitId,
        audioUrl,
        isYoutube);
  ...
  /**
   * Downloads an HTTP audio enclosure (e.g. from an RSS feed) by streaming the response body
   * directly to disk.
   *
  ...
  public Path downloadHttp(final String audioUrl, final UUID contentUnitId) {
    ...
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      log.debug(
          "HTTP download attempt {}/{} for contentUnit={}", attempt, MAX_RETRIES, contentUnitId);
      try {
        restClient
            .get()
            .uri(audioUrl)
            .exchange(
                (final var request, final var response) -> {
        ...
        log.warn(
            "HTTP download attempt {}/{} failed for contentUnit={}: {}",
            attempt,
            MAX_RETRIES,
            contentUnitId,
            e.getMessage());
  ...
  /**
   * Runs the given command up to {@link #MAX_RETRIES} times, deleting any partial output file
   * between failed attempts.
   */
  private void runWithRetry(
      final List<String> command, final Path outputPath, final String description) {
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
  ...
  /**
   * Starts the process, redirects stderr to the SLF4J logger, and waits for completion within
   * {@link #PROCESS_TIMEOUT_MINUTES} minutes.
   */
                  ...
                  try (final var is = process.getInputStream()) {
                    ...
                    while ((read = is.read(buf)) != -1) {
                      ...
                      while ((nl = line.indexOf("\n")) >= 0) {
                        log.debug(
                            "[{}] {}", command.getFirst(), line.substring(0, nl).stripTrailing());
                        line.delete(0, nl + 1);
                      }
                    }
                    if (!line.isEmpty()) {
  ...
  private void checkBinary(final String binary) {
    ...
    try {
      ...
      throw new AudioServiceException(
          "Dependency '"
              + binary
              + "' not found on PATH. Install it before starting the application.",
          e);
    }
  }
