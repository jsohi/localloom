...
/**
 * APP-77 — Detects YouTube, Apple Podcasts, Spotify, and RSS/Atom feed URLs and
 * resolves them to a {@link ResolvedPodcast} containing metadata and an episode list.
 *
 * <ul>
 *   <li>YouTube watch/short/playlist URLs → resolved via yt-dlp stub; video ID extracted.</li>
 *   <li>Apple Podcasts URLs → iTunes Lookup API, then RSS feed for episode list.</li>
 *   <li>Spotify show URLs → Spotify oEmbed for show title, then iTunes Search for RSS feed.</li>
 *   <li>All other URLs → treated as direct RSS/Atom feeds.</li>
 * </ul>
 */
@Service
public class UrlResolver {
     ...
     * returns stub metadata. Audio download is handled by {@link AudioService} via yt-dlp.
     */
     ...
     * iTunes Lookup API, then fetches the episode list from the RSS feed.
     */
     ...
     * then searches the iTunes API to find the corresponding podcast RSS feed.
     */
    ...
    private String extractRssArtwork(Element channel) {
        ...
        if (imageNodes.getLength() > 0) {
            return firstElementText((Element) imageNodes.item(0), "url");
        }
        return null;
    }
    ...
    /**
     * Parses an RFC-822 date string (standard in RSS feeds) to an {@link Instant}.
    ...
    /**
     * Parses an {@code itunes:duration} value into total seconds.
    ...
    private Integer parseDurationSeconds(String durationStr) {
        if (durationStr == null || durationStr.isBlank()) return null;
        try {
