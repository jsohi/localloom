...
public class EmbeddingService {
  ...
  /**
   * Chunks each fragment's text, wraps chunks in Spring AI Documents with metadata aligned to the
   * ChromaDB schema, and stores them.
   */
  public void embedContent(
      final UUID sourceId,
      final UUID contentUnitId,
      final String title,
    ...
    log.debug(
        "Embedding content for contentUnitId={} sourceId={} fragments={}",
        contentUnitId,
        sourceId,
        fragments.size());
    ...
    if (allDocuments.isEmpty()) {
      log.warn(
          "No documents generated for contentUnitId={}, skipping vectorStore.add", contentUnitId);
      return;
    }

    log.info(
        "Storing {} chunks for contentUnitId={} sourceId={}",
        allDocuments.size(),
        contentUnitId,
        sourceId);
  ...
  public List<Document> search(
    ...
    log.debug(
        "Searching query='{}' topK={} sourceIds={} sourceTypes={}",
        query,
        topK,
        sourceIds,
        sourceTypes);
  ...
  private String buildContextualText(
      final String title,
      final SourceType sourceType,
      final ContentType contentType,
